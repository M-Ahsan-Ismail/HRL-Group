...
class FetchTimeOffController(http.Controller):
    ...
    def Retrieve_Time_Off_Data(self, **kwargs):
        ...
        for rec in taken_leave_recs:
            ...
            print('Duration String: ', duration_str)
            if rec.holiday_status_id.request_unit == 'hour':  # Discuss It --->
                print('Yes Hited...')
                hour_from = float(rec.request_hour_from)
                hour_to = float(rec.request_hour_to)
                total_hours = hour_to - hour_from

                taken_value = round(total_hours / 8.0, 2)
            else:
        ...
        return request.render('bss_leave_request_portal_v16.time_off_controller_portal_view_id', {
