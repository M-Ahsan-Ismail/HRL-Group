...
from odoo.addons.bss_leave_request_portal_v16.controller.update_time_off_controller import UpdateTimeOffController
...
class CreateTimeOffController(http.Controller):
    ...
    def create_Time_Off_Data(self, **kwargs):
        ...
        if request.httprequest.method == 'POST':
            ...
            leave_request_id = request.env['hr.leave'].sudo().create(leave_vals)

            # Updating Id to fix 12pm---onwards time counting :
            update_vals = {
                'name': description,
                'holiday_status_id': time_off_type,
                'request_date_from': datetime.combine(from_date, time(0, 0)),
                'request_date_to': datetime.combine(request_date_to, time(23, 59, 59)),
                'request_unit_half': request_unit_half,
                'request_date_from_period': half_morning_or_evening if request_unit_half else False,
                'request_unit_hours': request_unit_hours,
                'request_hour_from': request_hour_from if request_unit_hours else False,
                'request_hour_to': request_hour_to if request_unit_hours else False,
            }

            hr_leave_rec_to_update_id = request.env['hr.leave'].search([('id', '=', leave_request_id.id)], limit=1)
            hr_leave_rec_to_update_id.sudo().write(update_vals)

            return request.redirect('/all/time/off')

        return request.render('bss_leave_request_portal_v16.time_off_create_form_template', {
            'all_types': all_types,
