...
class EmployeeExpenseController(http.Controller):
    ...
    def get_employee_expense(self, **post):
        ...
        for expense in expense_recs:
            all_expenses.append({
                ...
                'total_amount': expense.total_amount_currency,
                'state': expense.state,
        ...
        return request.render('bss_employee_expense_portal_v17.employee_expense_portal_id',
    ...
    def http_update_employee_expense(self, expense_id, **post):
        ...
        if expense.state in ['draft', 'reported', 'submitted']:
            update_vals = {
                'name': post.get('name'),
                'product_id': int(post.get('product')) if post.get('product') else None,
                'date': post.get('expense_date'),
                'total_amount_currency': float(post.get('total_amount')) if post.get('total_amount') else 0.0,
    ...
    def http_create_expense(self, **post):
        ...
        return request.render('bss_employee_expense_portal_v17.employee__create_expense_controller_id',
